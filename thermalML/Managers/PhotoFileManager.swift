...
class PhotoFileManager {
    ...
    private init() {
        Task {
            await createPhotosRootDirectory()
        }
    }
    ...
    private func createPhotosRootDirectory() async {
        let directoryURL = photosRootDirectory
        if !FileManager.default.fileExists(atPath: directoryURL.path) {
            do {
                try FileManager.default.createDirectory(at: directoryURL, withIntermediateDirectories: true, attributes: nil)
    ...
    private func createDirectory(for date: Date) async -> URL? {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
        let dateString = dateFormatter.string(from: date)
        let directoryURL = photosRootDirectory.appendingPathComponent(dateString)
    ...
    // MARK: - Save Photo
    func savePhoto(isOriginal: Bool, _ photoData: Data) async -> String? {
        let fileName = generateUniqueFileName(isOriginal: isOriginal)
        guard let directoryURL = await createDirectory(for: Date()) else {
            return nil
        }
        let fileURL = directoryURL.appendingPathComponent(fileName)
        do {
    ...
    // MARK: - Fetch Photo
    func fetchPhoto(withPath relativePath: String) async -> Data? {
        let fileURL = photosRootDirectory.appendingPathComponent(relativePath)
        do {
            return try Data(contentsOf: fileURL)
        } catch {
    ...
    // MARK: - Get Photo Info
    func fetchPhotoInfo(withPath relativePath: String) async -> [String: String] {
        let fileURL = photosRootDirectory.appendingPathComponent(relativePath)
        do {
            let attributes = try FileManager.default.attributesOfItem(atPath: fileURL.path)
            guard let fileSize = attributes[.size] as? NSNumber,
        ...
        // MARK: - Delete Photo
    func deletePhoto(withPath relativePath: String) async {
        let fileURL = photosRootDirectory.appendingPathComponent(relativePath)
        do {
            try FileManager.default.removeItem(at: fileURL)
        } catch {
