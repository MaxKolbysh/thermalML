 ...
 *  @note For a streamed images we do not recommend using rotate() due to possible performance degradation (a rotation can consume lot of processing power), it can be done without issue on a file-based FLIRThermalImage.
 ...
 *  @note Important: When rotating only the colorized pixels (UIImage), adding new measurements will require you to handle the difference in the displayed image and the actual FLIRThermalImage i.e. calculate the correct positions and redraw measurement and this can became really complicated.
 *  @note Warning: this is currently an experimental feature for live streaming and may not be fully functional.
 */
